...
interface ChatMessage {
  ...
  followUpButton?: { label: string; screen: ScreenType };
}
...
interface DemoLocaleItem {
  label: string;
  prompts: PromptLocale[];
  gmail?: { subject: string; body: string };
  pdf?: { client: string; service: string; date: string };
  sentEmails?: { subject: string; preview: string }[];
}

interface FollowUpLabels {
  viewEmail: string;
  viewInvoice: string;
  viewSentEmails: string;
}

// --- Build demos from locale data ---

function buildDemos(items: DemoLocaleItem[], followUpLabels: FollowUpLabels): Demo[] {
  return [
    {
      id: 'briefing',
      label: items[0]?.label ?? '',
      prompts: (items[0]?.prompts ?? []).map((p) => ({
        ...p,
        time: '8:01 AM',
      })),
    },
    {
      id: 'correo',
      label: items[1]?.label ?? '',
      prompts: (items[1]?.prompts ?? []).map((p, i) => ({
        ...p,
        time: '10:15 AM',
        ...(i === 0
          ? { followUpScreen: 'gmail' as ScreenType, followUpLabel: followUpLabels.viewEmail }
          : {}),
      })),
      gmail: {
    ...
    },
    {
      id: 'template',
      label: items[2]?.label ?? '',
      prompts: (items[2]?.prompts ?? []).map((p, i) => ({
        ...p,
        time: '2:30 PM',
        ...(i === 0
          ? { followUpScreen: 'gmail' as ScreenType, followUpLabel: followUpLabels.viewEmail }
    ...
    },
    {
      id: 'facturacion',
      label: items[3]?.label ?? '',
      prompts: (items[3]?.prompts ?? []).map((p, i) => ({
        ...p,
        time: '4:45 PM',
        ...(i === 0
          ? {
              pdfAttachment: { name: 'Factura_0892.pdf', size: '145 KB' },
              followUpScreen: 'pdf' as ScreenType,
              followUpLabel: followUpLabels.viewInvoice,
            }
          : {}),
      })),
      pdf: {
...
function ChatBubble({
  ...
  message: ChatMessage;
  onFollowUpClick?: (screen: ScreenType) => void;
}) {
  const isUser = message.from === 'user';
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} px-1 animate-msg-in`}>
      <div
        className={`max-w-[85%] rounded-xl px-3 py-2 shadow-sm text-[13px] leading-[1.45] ${
          isUser
            ? 'bg-[#D9FDD3] rounded-tr-sm text-gray-900'
      ...
      >
        ...
        {message.followUpButton && (
          <button
            onClick={() => onFollowUpClick?.(message.followUpButton!.screen)}
            className="mt-2 w-full rounded-lg border border-[#00A884]/30 bg-[#00A884]/5 px-3 py-2 text-center text-[12px] font-medium text-[#00A884] transition-colors hover:bg-[#00A884]/15"
          >
            {message.followUpButton.label}
          </button>
        )}
        <p className="mt-0.5 flex items-center justify-end gap-0.5 text-[10px] text-[#667781]">
          ...
          {isUser && (
            <svg
              className="h-[14px] w-[14px] text-[#53BDEB]"
              viewBox="0 0 16 15"
              fill="currentColor"
            >
...
function WhatsAppScreen({
  ...
  usedPrompts: Set<number>;
  onPromptClick: (idx: number) => void;
  onFollowUpClick: (screen: ScreenType) => void;
  ...
  const unusedPrompts = availablePrompts
    .map((p, i) => ({ ...p, idx: i }))
    .filter((p) => !usedPrompts.has(p.idx) && p.userMsg !== null);
      ...
      >
        {chatMessages.length > 0 && (
          <div className="mb-1 flex justify-center">
            <span className="rounded-lg bg-[#E1F2FB] px-3 py-1 text-[11px] text-[#5F7E8A] shadow-sm">
              {labels.today}
            </span>
          </div>
        )}
        {chatMessages.map((msg, i) => (
          <ChatBubble key={i} message={msg} onFollowUpClick={onFollowUpClick} />
        ))}
        {showTyping && <TypingIndicator />}
      </div>

      {/* Input bar with prompt chips */}
      <div className="bg-[#F0F0F0] px-2 py-2">
        {unusedPrompts.length > 0 ? (
          <div className="flex flex-wrap gap-1.5 px-1">
            {unusedPrompts.map((prompt) => (
              <button
                ...
                className="animate-chip-pulse rounded-full bg-[#00A884] px-3 py-1.5 text-[12px] font-medium text-white shadow-sm transition-all hover:bg-[#00997A] hover:shadow-md active:scale-95"
              >
                {prompt.chip}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 rounded-full bg-white px-4 py-2 text-[14px] text-[#667781]">
...
function GmailScreen({
  ...
  email: GmailData;
  labels: { sent: string; reply: string; forward: string; to: string };
  onBack: () => void;
}) {
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="px-4 pt-4 pb-2">
        <div className="flex items-start gap-2">
          <h3 className="flex-1 text-[16px] font-normal leading-tight text-[#202124]">
            {email.subject}
          </h3>
        </div>
      ...
      <div className="flex-1 overflow-hidden px-4 py-2">
        <p className="whitespace-pre-line text-[13px] leading-relaxed text-[#3c4043]">
          {email.body}
        </p>
      </div>
...
function PdfViewerScreen({
  ...
  onApprove: () => void;
  onViewEmails?: () => void;
  showViewEmails: boolean;
  viewEmailsLabel: string;
  labels: {
    invoice: string;
    client: string;
    service: string;
    ...
    <div className="flex h-full flex-col bg-[#525659]">
      ...
      <div className="flex items-center gap-3 bg-[#3c4043] px-4 py-3">
        ...
        <p className="flex-1 truncate text-[13px] font-medium text-white">
          {labels.invoice}_{pdf.number}.pdf
        </p>
        <svg
          className="h-5 w-5 text-white/80"
          fill="none"
      ...
      <div className="flex flex-1 items-start justify-center overflow-hidden p-4">
        <div className="w-full max-w-[260px] rounded bg-white p-5 shadow-lg">
          ...
          <div className="border-b border-gray-200 pb-3 text-center">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#C47B2B]">
              {labels.invoice}
            </p>
      ...
      <div className="border-t border-[#3c4043] bg-[#3c4043] px-4 py-3">
        {approved ? (
          <div className="space-y-2">
            ...
            {showViewEmails && onViewEmails && (
              <button
                onClick={onViewEmails}
                className="w-full rounded-xl border border-white/20 py-2.5 text-center text-[13px] font-medium text-white/90 transition-colors hover:bg-white/10"
              >
                {viewEmailsLabel}
              </button>
            )}
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onApprove}
              className="flex-1 rounded-xl bg-[#1B7A5A] py-3 text-center text-[13px] font-medium text-white transition-colors hover:bg-[#1a8f65]"
            >
              {labels.approve}
            </button>
          </div>
...
function EmailListScreen({
  ...
  emails: SentEmail[];
  labels: { sent: string; searchMail: string; billing: string };
  onBack: () => void;
}) {
  return (
    <div className="flex h-full flex-col bg-white">
              ...
              <div className="flex items-baseline justify-between gap-2">
                ...
                <span
                  className={`shrink-0 text-[11px] ${
                    email.highlighted ? 'font-bold text-[#202124]' : 'text-[#5f6368]'
                  }`}
                >
                  {email.time}
                </span>
...
export default function DemoVideos() {
  ...
  const followUpLabels = tObject<FollowUpLabels>('demos.followUpLabels');
  const demos = useMemo(() => buildDemos(demoItems, followUpLabels), [demoItems, followUpLabels]);

  const whatsappLabels = tObject<{
    online: string;
    messagePlaceholder: string;
    today: string;
  }>('demos.whatsapp');
  const gmailLabels = tObject<{
    sent: string;
    reply: string;
    forward: string;
    to: string;
  ...
  const [autoIntroPlayed, setAutoIntroPlayed] = useState(false);
  ...
  // Auto-intro: play first prompt when section becomes visible
  useEffect(() => {
    if (!hasBeenVisible || autoIntroPlayed || !activeDemo?.prompts?.[0]) return;
    clearTimers();

    const prompt = activeDemo.prompts[0];

    if (prompt.userMsg) {
      // User-initiated: show user message → typing → AI response
      const t1 = setTimeout(() => {
        setChatMessages([{ from: 'user', text: prompt.userMsg!, time: prompt.time }]);
      }, 500);

      const t2 = setTimeout(() => setShowTyping(true), 800);
      ...
      const t3 = setTimeout(() => {
        ...
        const aiMsg: ChatMessage = {
          ...
          text: prompt.aiMsg,
          time: prompt.time,
          pdf: prompt.pdfAttachment,
        };
        if (prompt.followUpScreen && prompt.followUpLabel) {
          aiMsg.followUpButton = {
            label: prompt.followUpLabel,
            screen: prompt.followUpScreen,
          };
        }
        setChatMessages((prev) => [...prev, aiMsg]);
        setUsedPrompts(new Set([0]));
        setAutoIntroPlayed(true);
      }, 2200);

      timersRef.current = [t1, t2, t3];
    } else {
      // AI-initiated (facturacion): typing → AI message directly
    ...
    return clearTimers;
  }, [hasBeenVisible, activeTab, autoIntroPlayed, activeDemo, clearTimers]);

  // Handle prompt chip click
  const handlePromptClick = useCallback(
    (promptIdx: number) => {
      const prompt = activeDemo.prompts[promptIdx];
      if (!prompt || !prompt.userMsg) return;

      setChatMessages((prev) => [
        ...prev,
        { from: 'user', text: prompt.userMsg!, time: prompt.time },
      ]);
      setUsedPrompts((prev) => { const next = new Set(prev); next.add(promptIdx); return next; });
  ...
  // Handle follow-up button click (inside chat bubble)
  const handleFollowUpClick = useCallback((targetScreen: ScreenType) => {
    setScreen(targetScreen);
    setScreenKey((k) => k + 1);
  }, []);

  // Handle back from follow-up screen
  ...
  // Handle PDF approve
  const handlePdfApprove = useCallback(() => {
    setPdfApproved(true);
  }, []);

  // Handle view sent emails (after PDF approval)
  const handleViewEmails = useCallback(() => {
    setScreen('email-list');
    setScreenKey((k) => k + 1);
  }, []);

  // Handle tab switch — reset everything
  ...
  const renderScreen = () => {
    switch (screen) {
      case 'whatsapp':
        return (
          <WhatsAppScreen
            ...
            availablePrompts={activeDemo.prompts}
      ...
      case 'gmail':
        return activeDemo.gmail ? (
          <GmailScreen email={activeDemo.gmail} labels={gmailLabels} onBack={handleBack} />
        ) : null;
      case 'pdf':
        return activeDemo.pdf ? (
          <PdfViewerScreen
            pdf={activeDemo.pdf}
            approved={pdfApproved}
            onApprove={handlePdfApprove}
            onViewEmails={handleViewEmails}
            showViewEmails={!!activeDemo.sentEmails?.length}
            viewEmailsLabel={followUpLabels.viewSentEmails}
            labels={pdfLabels}
            onBack={handleBack}
          />
        ) : null;
      case 'email-list':
        return activeDemo.sentEmails ? (
          <EmailListScreen
            emails={activeDemo.sentEmails}
            labels={gmailLabels}
            onBack={handleBack}
          />
        ) : null;
